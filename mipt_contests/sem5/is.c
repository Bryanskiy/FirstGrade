...
int IS_BST(struct tree_t* top, int min, int max);
...
int IS_BST(struct tree_t* top, int min, int max)
{
    ...
    return IS_BST(top->left, min, top->data - 1) && IS_BST(top->right, top->data + 1, max);
}

int main()
{
    struct tree_t *top = read_tree();
